...
const SignUp = () => {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        // Se não existir, cria um documento no Firestore
        await setDoc(doc(db, 'users', user.uid), {
          name: user.displayName || 'Nome padrão',
          email: user.email,
          profilePic: user.photoURL || '', // Adiciona a foto de perfil, se disponível
          createdAt: new Date(),
        });
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Adiciona dados adicionais no Firestore
      await setDoc(doc(db, 'users', user.uid), {
        name: name,
        email: email,
        createdAt: new Date(), // Data de criação
        profilePic: '',  // Você pode adicionar a URL de uma foto de perfil, se tiver
      });
  
      setSuccessMessage('Conta criada com sucesso!');
      setFormData({ name: '', email: '', password: '', confirmPassword: '' });
  
      // Redireciona após o cadastro e salvamento no Firestore
      setTimeout(() => {
